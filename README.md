...
To switch to the Claude API, put a key in `.env`:
...
142 tests covering booking rules (slot generation, double-booking, leave,
...
triage decisions, the Claude request/response handling, and the fallback path
...
Django views / forms  <-->  Django REST Framework
      ...
      |       assistant.py           facade: try Claude, fall back, log
      |       claude_provider.py     Anthropic SDK, structured outputs
...
Django 5.2 · Django REST Framework · Anthropic SDK · Bootstrap 5 ·
...
`DJANGO_SECRET_KEY` is generated by Render; `ANTHROPIC_API_KEY` is marked
...
- The Claude API needs internet access and an API key; without one the offline
  engine is used.
